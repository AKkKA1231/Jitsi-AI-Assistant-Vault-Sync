...
it('Should have Gemini Flash model cascade configured', () => {
  ...
  // Verify current active models are used (deprecated 1.x/2.x models removed)
  ...
  assert.ok(bgContent.includes('gemini-3.7-flash'), 'background.js must use gemini-3.7-flash');
  assert.ok(bgContent.includes('gemini-3.8-flash'), 'background.js must use gemini-3.8-flash');
  assert.ok(contentJs.includes('gemini-2.5-flash'), 'content.js must use gemini-2.5-flash');
  assert.ok(contentJs.includes('gemini-3.5-flash'), 'content.js must use gemini-3.5-flash');

  // Verify deprecated models are NOT present
  assert.ok(!bgContent.includes('gemini-1.5-flash'), 'background.js must NOT use deprecated gemini-1.5-flash');
  assert.ok(!bgContent.includes('gemini-2.0-flash\''), 'background.js must NOT use retired gemini-2.0-flash');
});
...
await itAsync('Should verify live Gemini API connection if key is provided', async () => {
  ...
  const models = ['gemini-2.5-flash', 'gemini-3.5-flash', 'gemini-3.7-flash', 'gemini-3.8-flash'];
